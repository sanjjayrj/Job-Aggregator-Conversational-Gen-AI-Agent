...
from matcher import main as match_jobs
import os
import openai
import re
import json
            ...
            try:
                ...
                if os.path.exists(scraped_jobs_file):
                    ...
                    if ranked_jobs is not None:
                        ...
                        top_jobs = filtered_jobs.head(5).to_dict(orient='records')
...
def dashboard():
    ...
    return render_template('dashboard.html', jobs=[], job_history=job_history, resume_path=resume_path)
...
def get_top_jobs():
    try:
        if status["matching_complete"] and os.path.exists(top_jobs_file):
            ...
            print("Top jobs data:", top_jobs) #.to_dict(orient="records"))
            return jsonify({"jobs": top_jobs}) #.to_dict(orient="records")})
        else:
    ...
    def rerun_matcher():
        try:
            if os.path.exists(scraped_jobs_file):
                ranked_jobs = match_jobs(resume_json_path=parsed_resume_file, jobs_csv_path=scraped_jobs_file)
                if ranked_jobs is not None:
                    top_jobs = ranked_jobs.head(5).to_dict(orient='records')
                    pd.DataFrame(top_jobs).to_json(top_jobs_file, orient="records")
                status["matching_complete"] = True
        except Exception as e:
...
def detect_update_intent(user_message, openai_api_key):
    ...
    prompt = """
        ...
        - If the user is asking about job-related information but not requesting updates, return "general".
